...
def run_multi_company_researcher(companies_list):
    ...
    for company in companies_list:
        ...
        # ב. שלב האינטרנט
        web_prompt = f"""
        אתה חוקר נתונים מיומן. המשימה שלך היא לגבש רשימת סניפים סופית עבור חברת "{company}" בישראל.
        
        להלן נתונים שכבר נמצאו ב-Google Maps:
        {maps_data}
        
        הנחיות לביצוע:
        1. חפש באינטרנט סניפים נוספים או אמת את הקיימים.
        2. עבור כל סניף, קבע את המקור (field: "source") לפי הכללים הבאים:
           - אם הסניף הגיע מרשימת המפות שסיפקתי לך, כתוב "Google Maps".
           - אם מצאת את הסניף בחיפוש באינטרנט, כתוב את שם האתר (למשל: "אתר רשמי", "דפי זהב") או פשוט "Google Search".
        
        החזר אך ורק מערך JSON תקני (ללא Markdown) שבו כל אובייקט מכיל:
